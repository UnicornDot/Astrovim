...
function M.core_mappings(mappings)
  ...
  if maps then
    ...
    maps.n["<S-TAB>"] =
      { function() astrobuffer.nav(-(vim.v.count > 0 and vim.v.count or 1)) end, desc = "Previous buffer" }
    ...
    if vim.fn.executable "btm" == 1 then maps.n["<Leader>tt"] = { utils.toggle_btm(), desc = "ToggleTerm btm" } end
    ...
    maps.n["<M-1>"] = {
      function() require("snacks.terminal").toggle(nil, { win = { position = "bottom", height = 0.15 } }) end,
      desc = "Snacks terminal horizontal",
    }
    maps.t["<M-1>"] = maps.n["<M-1>"]
    maps.n["<M-2>"] = {
      function() require("snacks.terminal").toggle(nil, { win = { position = "right", width = 0.4 } }) end,
      desc = "Snacks terminal vertical",
    }
    maps.t["<M-2>"] = maps.n["<M-2>"]
    maps.n["<M-3>"] = {
      function() require("snacks.terminal").toggle(nil, { win = { position = "float", border = "rounded" } }) end,
      desc = "Snacks terminal float",
    }
    maps.t["<M-3>"] = maps.n["<M-3>"]
    maps.n["<C-\\>"] = {
      function() require("snacks.terminal").toggle(nil, { win = { position = "float", border = "rounded" } }) end,
      desc = "Snacks terminal float large",
    }
    maps.t["<C-\\>"] = maps.n["<C-\\>"]
    ...
    maps.n["<Leader>bC"] = { function() astrobuffer.close(0, true) end, desc = "Force close buffer" }
...
function M.lsp_mappings(mappings)
  ...
  if maps then
    ...
    maps.i["<C-h>"] = { function() vim.lsp.buf.signature_help() end, desc = "Signature help", cond = "textDocument/signatureHelp", }
    maps.n["grr"] = { function() snacks.picker.lsp_references() end, desc = "vim.lsp.buf.reference", }
